...
const Controls: React.FC<ControlsProps> = ({ 
  ...
  const styles: Record<string, React.CSSProperties> = {
    ...
    btnGroup: { display: 'flex', gap: '0.75rem', alignItems: 'center' },
    playBtn: {
    ...
    labelRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '1rem' },
    ...
    themeWrap: { display: 'flex', gap: '0.5rem' },
    dot: { width: '22px', height: '22px', borderRadius: '50%', border: '2px solid transparent' }
  };
      ...
      <div style={styles.btnGroup}>
        <button onClick={onTogglePlay} style={styles.playBtn}>
          {status === ReaderStatus.PLAYING ? 
            ...
            <svg width="22" height="22" fill="currentColor" viewBox="0 0 24 24" style={{marginLeft:'2px'}}><path d="M8 5v14l11-7z"/></svg>}
        </button>
        <button onClick={onReset} style={styles.resetBtn}>
          <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
        </button>
      </div>
